...
func pick_up(by: Node3D, with_controller: XRController3D) -> void:
	...
	print("pickable.gd: pick_up(): fired! ", with_controller) #does this fire when an extended src calls the same?... isn't that what super() is for??
	...
	print("pickable.gd: pick_up(): con is ",picked_up_by)
...
func let_go(p_linear_velocity: Vector3, p_angular_velocity: Vector3) -> void:
	# Skip if idle
	print("pickable.gd: let_go(): fired")
